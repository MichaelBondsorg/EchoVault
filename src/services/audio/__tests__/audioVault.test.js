...
describe('audioVault', () => {
  ...
  it('cleanupExpired deletes recordings older than 7 days but keeps fresh ones', async () => {
    const { id: oldId } = await audioVault.saveRecording(OWNER, 'T0xE', 'audio/webm');
    ...
    const { id: freshId } = await audioVault.saveRecording(OWNER, 'RlJFU0g=', 'audio/webm');
  ...
  it('never throws when storage fails — returns { error: io }', async () => {
    ...
    expect(result).toEqual({ error: 'io' });
    spy.mockRestore();
  });

  it('returns { error: quota } and leaves no orphan when the index write fails after a successful blob write', async () => {
