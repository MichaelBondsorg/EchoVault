...
describe('Verification: Off-Palette Color Audit', () => {
  ...
  it('should have zero off-palette color class instances outside safety files', () => {
    ...
    for (const file of files) {
      ...
      const lines = content.split('\n');
      let inSafelistBlock = false;

      lines.forEach((line, idx) => {
        if (isComment(line)) return;
        if (line.includes('@color-safe')) return;
        // Skip colorMap.js TAILWIND_SAFELIST block (multi-line template literal)
        if (file.endsWith('colorMap.js')) {
          if (line.includes('TAILWIND_SAFELIST')) inSafelistBlock = true;
          if (inSafelistBlock) {
            // End of safelist block: closing backtick
            if (inSafelistBlock && line.includes('`') && !line.includes('TAILWIND_SAFELIST')) {
              inSafelistBlock = false;
            }
            return;
          }
        }
