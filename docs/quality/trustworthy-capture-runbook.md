...
| `style-src` | `'self' 'unsafe-inline'` | React's `style={{...}}` (23 files) and every animation library in use (`framer-motion` in 71 files, `@dnd-kit`, `@xyflow/react`) set styles via the CSSOM (`element.style.x = ...`), which Chrome/Safari/Firefox do **not** gate behind `style-src` — only the declarative `style="..."` HTML attribute and `<style>`/`<link rel=stylesheet>` elements are. A repo-wide grep found no `dangerouslySetInnerHTML` writing a `style=` attribute and no `setAttribute('style', ...)`; the one `el.style.cssText = ...` site (`useWakeLock.js`) is also a CSSOM write, not an attribute write. HOWEVER — reviewer correction (2026-07-24): `index.html` ships an inline `<style>` block (base styles, dark-mode body background, reduced-motion CSS, safe-area insets), and inline `<style>` ELEMENTS are gated by `style-src`. So `'unsafe-inline'` IS load-bearing for first-party code today: dropping it without first hashing or externalizing that block will silently kill those styles (including the reduced-motion accessibility CSS). It's kept anyway as a deliberate, low-risk hedge: style-only injection is a far weaker vector than script injection (no code execution, no data exfil beyond CSS-based side channels), and several dependencies (Radix portals, `@xyflow/react` node positioning) couldn't be exhaustively verified for attribute-style writes without live testing, which isn't available pre-deploy. Tightening this later requires FIRST externalizing/hashing index.html's <style> block, then live-browser verification of the dependency hedge — it is NOT a drop-in follow-up. |
...
Storage (`firebasestorage.googleapis.com`) — `storageBucket` is configured. ALSO: the durable-capture upload-ticket path (issueCaptureUploadTicket -> signed GCS PUT) will need `storage.googleapis.com` in connect-src the day a client consumer wires it — currently a verified-dead path (audioVault is local-only); add the host WITH that feature, never before
...
- **`style-src 'unsafe-inline'` is LOAD-BEARING for index.html's inline <style> block (see table above), and additionally a hedge — not a proven
