...
// Server-owned single-pass post-save analysis (plan task C3), behind the
// `serverAnalysisOrchestrator` flag. When ON this OWNS classification+analysis
// end-to-end so the client no longer runs its fragile promise chain (which dies
// on app suspension and classifies twice). Guard chain, in order:
//   flag ON -> entry has text and isn't 'disabled' -> consent allowed ->
//   claim a one-time processing marker (dedups at-least-once redelivery) ->
//   runEntryAnalysis (which re-checks consent per stage, guards stale edits,
//   and publishes provenance-stamped results in a single transaction).
export const onEntryCreatedAnalysis = onDocumentCreated(
  {
    document: 'artifacts/{appId}/users/{userId}/entries/{entryId}',
    secrets: [geminiApiKey, openaiApiKey],
  },
  async (event) => {
    ...
    const entry = { id: entryId, ...event.data.data() };

    // Skip explicitly-disabled entries and entries with no text to analyse.
    if (entry.analysisStatus === 'disabled') return { skipped: true, reason: 'disabled' };
    if (!entry.text || typeof entry.text !== 'string') {
      return { skipped: true, reason: 'no-text' };
    }

    // Server-authoritative consent gate (fails closed on an unreadable doc).
    if (!(await isAiAllowed(db, userId, { entrySnapshot: entry }))) {
      console.log(JSON.stringify({ type: 'consent-denied', uid: userId, job: 'onEntryCreatedAnalysis' }));
      return { skipped: true, reason: 'ai-consent-revoked' };
    }

    // At-least-once delivery: claim a one-time marker so a redelivered create
    // event doesn't run analysis twice.
    const claimed = await claimProcessingMarker(db, ref, 'processing.analysisStartedAt');
    if (!claimed) return { skipped: true, reason: 'already-processing' };

    try {
      const result = await runEntryAnalysis({
        db,
        entryRef: ref,
        entry,
      ...
      console.error(`[onEntryCreatedAnalysis] failed for ${entryId}:`, error?.message);
      return { success: false, error: error?.message };
    }
  }
);
...
export const pendingEntryCleanup = onSchedule(
  ...
  async () => {
    ...
    for (const docSnap of snap.docs) {
      ...
      // When the server orchestrator flag is ON, route stuck entries through the
      // single-pass orchestrator instead of the legacy inline path. Reuses the
      // lease won above; the orchestrator re-checks consent per stage, guards
      // stale edits, and clears the lease in its final publish transaction.
      if (await getServerFlag(db, 'serverAnalysisOrchestrator', false)) {
        if (Object.keys(crisisFields).length > 0) {
          await docSnap.ref.update(crisisFields);
        }
        try {
          const result = await runEntryAnalysis({
            db,
            entryRef: docSnap.ref,
            entry: { id: docSnap.id, ...data },
            apiKeys: { gemini: apiKey, openai: undefined },
            logStage,
          });
          if (result.outcome === 'published') analyzed++;
          else if (result.outcome === 'failed') failed++;
        } catch (e) {
          failed++;
          console.error(`[pendingEntryCleanup] orchestrator failed for ${docSnap.id}:`, (e?.message || String(e)).slice(0, 200));
        }
        continue;
      }
