...
| 2026-07-23 | **R4 Phase 2 design defaults P2-D1..D8 ratified by controller** (plan header, `docs/superpowers/plans/2026-07-23-r4-phase2-trustworthy-synthesis.md`; same autonomous-session/veto-window convention as R4 Phase 1's D1-D9): P2-D1 the writer/verifier pair authors ONLY `wording`, never `evidence`/`analysisPlan`/numbers; P2-D2 `LLM_WRITER_ENABLED` is a code constant (not a `config/flags` field) so a real deploy, not a remote toggle, is required to let an LLM author any user-facing claim text; P2-D3 the fallback to deterministic wording on any writer/verifier failure is unconditional and silent (never blocks or surfaces an error for an otherwise-eligible claim); P2-D4 writer and verifier MUST default to different models (independence of the entailment check is structural, not incidental); P2-D5 the unified `ClaimFeed` replaces BOTH legacy Quick Insights and AI Insights (Nexus) sections together, not incrementally, when `insightClaims` is ON — one feed, not two feeds side by side; P2-D6 Ask Journal and reports ground on verified claims only, never Nexus prose, for anything claims now cover; P2-D7 the DR's ≥80% comprehension gate (integrity-ladder gate 8) is a pre-broad-release PROCESS item, not Phase 2 code — no quiz surface is built; P2-D8 `experiment_result` reuses the existing claim schema/lineage machinery rather than a parallel experiment-specific claim shape | Same rationale posture as every prior phase's ratified-defaults row: ship on a controller-set default with an explicit veto window rather than blocking implementation on a synchronous decision from Michael for choices where the plan already reasoned through the tradeoff | Michael vetoes any of P2-D1..D8 before `LLM_WRITER_ENABLED` is ever flipped, or before broad release for P2-D7 |
