...
| 2026-07-22 | **Implementation-time choice, FOR MICHAEL'S CONFIRMATION (not a directive — GR1's own judgment call):** a dismissed `revisit_queue` item never trips the weekly-cadence gate — only a live (queued/shown) selection does | A user who dismissed this week's card shouldn't thereby be blocked from a fresh one sooner; per-entry re-suppression is `revisit_exclusions`' job | Michael wants a dismissal itself to also pause the week |
